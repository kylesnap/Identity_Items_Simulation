...
run_rep <- function(cell, pmat, y, design_mat) {
  categories <- make_x_v_cpp(cell$n, cell$probs, pmat[1,], pmat[2,], pmat[3,])
  colnames(design_mat) <- c("ACT_INT", "ACT_Z", "ACT_XW", "ACT_XX")
  design_mat[,2] <- make_z(categories$X, cell$delta)
  design_mat <- add_groups(design_mat, categories$X)
  Y <- (design_mat %*% cell$beta) + rnorm(cell$n, sd = cell$sigma)
  mod_act <- fastLmPure(X = design_mat, y = Y, method = 3)$coefficients
  colnames(design_mat) <- c("OBS_INT", "OBS_Z", "OBS_XW", "OBS_XX")
  design_mat <- add_groups(design_mat, categories$V)
  mod_obs <- fastLmPure(X = design_mat, y = Y, method = 3)$coefficients
  list2(
    !!!mod_act,
    !!!mod_obs,
    !!!categories$counts
...
run_cell <- function(cell, pmat, reps = 1) {
  ...
  map(
    c(1:reps),
    \(i) run_rep(cell, pmat, Y, design_mat)
  )
}
